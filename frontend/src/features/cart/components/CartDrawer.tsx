    ...
    <>
      ...
      >
        <header className="flex items-center justify-between border-b border-zinc-200 px-5 py-4">
          ...
          <button onClick={() => setDrawerOpen(false)} className="rounded-full p-2 text-zinc-700 hover:bg-zinc-100">
            <X size={18} />
          </button>
        </header>
        ...
        <footer className="space-y-4 border-t border-zinc-200 px-5 py-4">
          <div className="flex items-center justify-between text-sm text-zinc-600">
            <span>Subtotal</span>
            <span className="text-base font-semibold text-zinc-900">${total.toFixed(2)}</span>
          </div>
          <button
            onClick={() => {
              setDrawerOpen(false);
